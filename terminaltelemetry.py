...
from device_discovery import DeviceDiscovery  # New import
from device_discovery_gui3 import DiscoveryFrame
from terminal_pfc import TerminalWithPrecheck
...
class App(ttk.Frame):
    def __init__(self, parent, ssh_config, ui_config, log_file):
        ...
        # sessions_menu.add_command(label="Edit", command=self.edit_session)  # Implement edit_session function
        # sessions_menu.add_command(label="List", command=self.list_sessions)  # Implement list_sessions function
    ...
    def create_terminal_tab(self, host, port, username, password):
        ...
        try:
            # Use DiscoveryFrame with the new TerminalWithPrecheck class
            term_frame = DiscoveryFrame(
                tab_frame,
                ssh_config=ssh_config,
                terminal_class=TerminalWithPrecheck,  # Changed from Terminal to TerminalWithPrecheck
                log_file=self.log_file,
                **self.ui_config
            )
            term_frame.pack(expand=1, fill='both')

            # Add the tab only after successful connection
            self.tab_control.add(tab_frame, text=host, image=tab_id)
            self.widget_to_uuid[tab_frame] = tab_id

            # Store the session using the terminal reference
            self.sessions[str(tab_id)] = term_frame.terminal

            return term_frame.terminal

        except ConnectionError as e:
            print(f"Connection failed: {str(e)}")  # Added logging for debugging
            tab_frame.destroy()
            return None
...
if __name__ == "__main__":
    ...
    root.title("SSH Terminal Telemetry")

    app = App(root, ssh_config, ui_config, log_file)
    try:
        app.pack(expand=True, fill="both")
    except:
        # may fail if no open terminal, i.e. durring shutdown
        pass
