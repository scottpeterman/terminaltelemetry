...
class SessionManager {
    ...
    initializeWebChannel() {
        console.log('SessionManager: Initializing WebChannel...');
        new QWebChannel(qt.webChannelTransport, (channel) => {
            console.group('SessionManager: WebChannel connected');
    ...
    initializeModalHandling() {
        ...
        const telemetryConnectBtn = document.getElementById('telemetry-connect');
        ...
        console.dir({
            elements: {
                connectBtn: !!telemetryConnectBtn,
                modal: !!modal,
                form: !!form
            }
        }, { depth: null });
...
class TelemetrySession extends BaseSession {
    constructor(sessionId, messageRouter) {
        ...
        this.debugId = Math.random().toString(36).substring(7);
        this.interfacePanel = new InterfacePanel(this.debugId);
        ...
        console.log('TelemetrySession initialized with states:', this.widgetStates);
        this.deviceState = 'unknown';
        console.log('TelemetrySession initialized:', this);
    }
    ...
    initializeTelemetry() {
        ...
        this.elements = {
            ...
            environment: document.querySelectorAll('[data-env]')
        };
...
updateEnvironmentDisplay(data) {
    ...
    if (data.cpu && this.elements.cpuStatus) {
        this.elements.cpuStatus.innerHTML = `
            <div class="env-item">
                <span>CPUs: ${data.cpu.num_cpus || 1}</span>
                <span>Usage: ${data.cpu.average_usage?.toFixed(1)}%</span>
            </div>
        `;
    }
    ...
    if (data.memory && this.elements.memoryStatus) {
        this.elements.memoryStatus.innerHTML = `
            <div class="env-item">
                ...
                <span>Usage: ${data.memory.usage_percent?.toFixed(1)}%</span>
            </div>
        `;
    }
        ...
        this.elements.temperatureStatus.innerHTML = data.temperature
            .map(temp => `
                <div class="env-item ${temp.critical ? 'critical' : temp.alert ? 'warning' : ''}">
                    <span>${temp.location}: ${temp.temperature}°C</span>
                </div>
            `).join('');
    }
        ...
        this.elements.powerStatus.innerHTML = data.power
            .map(psu => `
                <div class="env-item ${psu.status ? '' : 'warning'}">
                    <span>PSU ${psu.id}: ${psu.status ? 'OK' : 'FAIL'}</span>
                    ${psu.output ? `<span>${psu.output}W / ${psu.capacity}W</span>` : ''}
                </div>
            `).join('');
    }
        ...
        this.elements.fansStatus.innerHTML = data.fans
            .map(fan => `
                <div class="env-item ${fan.status ? '' : 'warning'}">
                    <span>${fan.location}: ${fan.status ? 'OK' : 'FAIL'}</span>
                </div>
            `).join('');
    }
}
...
async requestAllTelemetryData() {
    try {
        // Chain of promises for sequential execution
...
cleanup() {
    ...
    connect(connectionInfo) {
        ...
        this.term.clear();
        this.term.setOption('fontSize', 30);
        this.term.fitAddon();
